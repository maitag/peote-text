...
@:allow(peote.text.packed) class PageP {
	...
	var pageLines = new Array<PageLineP>();
	public var length(get, never) : Int;
	public inline function get_length():Int return pageLines.length;
	public inline function getPageLine(i:Int):PageLineP return pageLines[i];
	inline function setLine(i:Int, line:PageLineP) pageLines[i] = line;
	inline function pushLine(line:PageLineP) pageLines.push(line);
	inline function resize(newLength:Int) {
		pageLines.splice(newLength, pageLines.length - newLength);
	}
	inline function spliceLines(pos:Int, len:Int):Array<PageLineP> {
		return pageLines.splice(pos, len);
	}
	inline function append(a:Array<PageLineP>) {
