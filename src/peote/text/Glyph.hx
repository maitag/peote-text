...
class GlyphMacro
{
	...
	static public function buildClass(className:String, classPackage:Array<String>, stylePack:Array<String>, styleModule:String, styleName:String, styleSuperModule:String, styleSuperName:String, styleType:ComplexType, styleField:Array<String>):ComplexType
	{
		...
		if ( !Macro.typeAlreadyGenerated(fullyQualifiedName) )
		{
			...
			Context.defineModule(fullyQualifiedName, [ getTypeDefinition(className, styleModule, styleName, styleType) ]);
		}
		return TPath({ pack:classPackage, name:className, params:[] });
	}

	static public function getTypeDefinition(className:String, styleModule:String, styleName:String, styleType:ComplexType):TypeDefinition
	{
		...
		var c = macro class $className implements peote.view.Element
		{
			public var char(default, null):Int = -1;
			public function new() {}
			
			inline function setStyle(glyphStyle: $styleType) {
				$b{ exprBlock }
			}			
		}
		...
		{			
			...
			c.fields.push({
				name: "w",
				meta: [{name:"sizeX", params:[], pos:Context.currentPos()} //,{name:"varying", params:[], pos:Context.currentPos()},
					],
				access: [Access.APrivate],
				kind: FieldType.FVar(macro:Float, macro 0.0),
				pos: Context.currentPos(),
